...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                name="id", type=openapi.TYPE_STRING, in_=openapi.IN_PATH, description="View ID"
            ),
        ],
    ),
)
...
class ViewAPI(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return View.objects.filter(project__organization=self.request.user.active_organization).order_by(
            "order", "id"
        )
